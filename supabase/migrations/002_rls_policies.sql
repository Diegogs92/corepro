...
CREATE OR REPLACE FUNCTION has_role(required_role rol_usuario)
RETURNS BOOLEAN AS $$
BEGIN
    RETURN EXISTS (
        SELECT 1 FROM usuarios
...
CREATE OR REPLACE FUNCTION has_any_role(required_roles rol_usuario[])
RETURNS BOOLEAN AS $$
BEGIN
    RETURN EXISTS (
        SELECT 1 FROM usuarios
...
CREATE POLICY "pagos_sueldos_insert" ON pagos_sueldos FOR INSERT
    WITH CHECK (has_any_role(ARRAY['ADMIN', 'GERENTE']::rol_usuario[]));

CREATE POLICY "pagos_sueldos_update" ON pagos_sueldos FOR UPDATE
    USING (has_any_role(ARRAY['ADMIN', 'GERENTE']::rol_usuario[]));
